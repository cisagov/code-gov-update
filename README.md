...
```console
docker run cisagov/code-gov-update:0.1.3
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      update:
        image: 'cisagov/code-gov-update:0.1.3'
        init: true
        environment:
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/code-gov-update:0.1.3
    ```
...
|-----------|-------------|
|`cisagov/code-gov-update:0.1.3`| An exact release version. |
...
```console
docker build \
  --tag cisagov/code-gov-update:0.1.3 \
